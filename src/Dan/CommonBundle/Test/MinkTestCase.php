...
use Dan\CommonBundle\Test\Exception\ElementNotFoundException;
...
class MinkTestCase extends WebTestCase
{
    ...
    public function find(Session $session, $selector, $el=null)
    {
        try {
            $el = $this->checkFind($session, $selector, $el);
        } catch (ElementNotFoundException $e ) {
            $this->throwElementNotFoundException($session, $selector);
        }
        return $el->find('css', $selector);
    }
    
    public function findAll($session, $selector, $el=null)
    {
        try {
            $el = $this->checkFind($session, $selector, $el);
        } catch (ElementNotFoundException $e ) {}
        return $el->findAll('css', $selector);
    }

    public function checkFind($session, $selector, $el=null)
    {
        if (!$el) {
            $el = $session->getPage();
        }
        if (!$el->has('css', $selector)) {
